...
一个关键工程边界是：简历、JD 和自我介绍不会被当作一坨原始脏数据直接塞进面试 prompt。上传材料先经过解析、规则抽取、审计摘要、短期 artifact 和 session anchor/RAG 分层，运行时 LLM 看到的是结构化字段、候选锚点和按预算召回的片段。
...
| Resume/JD parse artifacts | 短期 artifact + 解析审计摘要 | 隔离原始上传材料，保留可追溯解析结果，再供 session setup 和候选人锚点使用。 |
| RAG / anchors | Chroma + knowledge files | 支撑题库、简历锚点、技能卡和检索上下文。 |
| 本地匿名历史 | Browser storage | 兼容未登录体验，不等同于账号记录。 |

这个分层避免把所有状态塞进前端或一次性 prompt，也避免让 LLM 直接面对未治理的原始材料；生产问题可以按“账号、会话、解析 artifact、额度、trace、运行时依赖”分别排查。
...
- 真人基线评估集：先保留 trace、replay、credibility、reviewed acceptance checks 和人工标注入口；等 20-50 条专家标注样本具备后，再做相对 baseline 的一致率/漂移率评估。
...
- 不把当前模型评分包装成“真人准确率”，评估层先埋基线对比钩子，等专家样本到位后再给相对改进数字。
