...
- **R3 Right to be forgotten.** A candidate-driven delete must wipe `session_anchor_chunks`, setup snapshots / current question payloads containing resume- or self-intro-derived material, short-lived parse artifacts, self-intro anchor artifacts, and any cached artifacts. Dedicated admin endpoint + dedicated test.
...
## Task 4: Parse Artifact Binding + Session Anchor Vectorization
...
Expected: parse endpoints return source artifacts, session start binds artifacts to `session_id`, resume vectorize succeeds, long self-intro anchor cards vectorize, failed/expired/missing artifacts degrade to rule anchor, self-intro failures degrade to profile-only, and Mode D/short self-intro skip.

- [ ] Commit.

```bash
git add ai-interviewer/backend/app/services/resume_parse_artifacts.py ai-interviewer/backend/app/services/session_anchor_vectorize.py ai-interviewer/backend/app/api/v1/interview.py ai-interviewer/backend/app/services/resume_parse_jobs.py ai-interviewer/backend/app/engine/agents/self_intro.py ai-interviewer/backend/app/engine/workflow/nodes/self_intro.py ai-interviewer/backend/app/engine/workflow/state.py ai-interviewer/backend/tests/unit/test_resume_parse_artifacts.py ai-interviewer/backend/tests/unit/test_session_anchor_vectorize.py ai-interviewer/backend/tests/unit/test_self_intro_anchor_cards.py ai-interviewer/backend/tests/unit/test_resume_rag_session_binding.py ai-interviewer/backend/tests/unit/test_interview_setup_api_contract.py ai-interviewer/frontend/src/lib/api/types.ts ai-interviewer/frontend/src/components/interview/SetupForm.tsx ai-interviewer/frontend/src/lib/storage/setupDrafts.ts ai-interviewer/frontend/tests/resumeUpload.test.js
...
```
  ...
  - In Python, split hits by `source_type` and apply source quotas: `session_anchor_top_k_resume` resume hits plus `session_anchor_top_k_self_intro` self-intro hits. P0 defaults are `2 + 1`.
  - If all hits are below threshold or all source quotas are empty, set `fallback_reason="low_score"` or `"empty"`.
  - If the rule anchor's `project_name` is non-empty and matches a resume hit's `project_name`, mark the hit as `deduped=True` and render only the text body without the project-label header.
...
- [ ] In `retrieve_candidate_anchors`, when a resume hit's `project_name == rule_anchor.project_name`, set `hit.deduped = True` and render only the text body without the project-label header so the prompt does not say the same project twice.
...
- [ ] Add these routes (all behind `Depends(require_admin_token)`):
  ...
  - `DELETE /admin/sessions/{session_id}/anchor-data` - subject deletion (R3): delete all `session_anchor_chunks` for the session, remove any remaining parse artifacts and self-intro anchor artifacts for the session/source ids when present, scrub resume- and self-intro-derived fields from `InterviewSession.setup_snapshot`, `InterviewSession.current_question`, and `GenerationTrace.state_snapshot`.
...
- [ ] CLI: `python -m app.scripts.cleanup_session_anchor_chunks`
  ...
  - Iterates `session_anchor_chunks` where `expires_at < now()`.
...
| D | n/a | n/a | Minimal resume never enters vector store |
| SI | >= 50% | >= 10 | Long self-intro anchor cards |
...
- [ ] Write the runbook `RESUME_RAG_ROLLOUT.md` covering:
  ...
  - Rollback procedure: flip `resume_rag_mode=off` and (optionally) wipe session anchor chunks.
